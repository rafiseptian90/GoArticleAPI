...
func (repository ArticleRepository) GetTrendingArticlesByTags(tags []string) []models.Article {
	...
	repository.DB.Preload("User.Profile").Preload("Tags").Where("EXISTS (SELECT * FROM article_tags WHERE article_tags.article_id = articles.id AND article_tags.tag_id IN (?))", tags).Where("published_at BETWEEN ? AND ?", lastWeek, currentTime).Order("seen desc").Find(&articles)
